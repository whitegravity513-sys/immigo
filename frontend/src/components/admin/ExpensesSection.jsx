  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 border-b border-slate-200">
        ...
        >
          <Building2 size={16} /> Track By Client
        </button>
      </div>
      ...
      {/* TAB 3: TRACK BY CLIENT */}
      {activeTab === "clients" && (
        <div className="space-y-6">
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-5 flex flex-wrap items-center justify-between gap-4">
            <div>
              <h4 className="text-base font-black text-slate-800">Track Expense History According to Client</h4>
              <p className="text-xs text-slate-500">Analyze total company expenditure billable or spent on each specific client</p>
            </div>
            <div className="flex items-center gap-3">
              <label className="text-xs font-bold text-slate-600">Select Client:</label>
              <select
                value={selectedClientId}
                onChange={e => setSelectedClientId(e.target.value)}
                className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 text-xs font-bold text-slate-700 outline-none focus:border-blue-500"
              >
                <option value="">-- Choose Client --</option>
                {safeClients.map(c => (
                  <option key={c._id} value={c._id}>{c.name || c.companyName}</option>
          ...
          {selectedClientId ? (
            clientLoading ? (
              <div className="bg-white rounded-2xl border border-slate-200/60 p-12 text-center text-slate-400 font-semibold text-sm">
                Loading client expenditure logs...
              </div>
            ) : clientExpenseData ? (
              <div className="space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="bg-white p-5 rounded-2xl border border-blue-100 shadow-xs">
                    <span className="text-xs font-bold text-slate-400 uppercase">Client Total Spend</span>
                    <h3 className="text-3xl font-black text-blue-600 mt-1">
                      ₹{Number(clientExpenseData.totalSpent || 0).toLocaleString("en-IN")}
                    </h3>
                    <p className="text-xs text-slate-500 mt-1">Across {clientExpenseData.count || 0} transaction records</p>
                  </div>
                  <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs flex items-center justify-between">
                    <div>
                      <span className="text-xs font-bold text-slate-400 uppercase">Client Name</span>
                      <h3 className="text-xl font-black text-slate-800 mt-1">
                        {clientExpenseData.client?.name || clientExpenseData.client?.companyName}
                      </h3>
                      <p className="text-xs text-slate-500">{clientExpenseData.client?.email || "No email on file"}</p>
                    </div>
                    <Building2 size={36} className="text-blue-500 opacity-60" />
                  </div>
                </div>

                <div className="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
                  <div className="px-6 py-4 border-b border-slate-100">
                    <h5 className="text-sm font-black text-slate-800">Expenditure Log For This Client</h5>
                  </div>
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse min-w-[650px]">
                      <thead>
                        <tr>
                          {["Date", "Description / Vendor", "Type", "Amount", "Receipt"].map(h => (
                            <th key={h} className="px-4 py-3.5 text-slate-500 font-bold uppercase text-[10px] tracking-wider bg-slate-50/50 border-b border-slate-100">
                              {h}
                            </th>
                          ))}
                      ...
                      <tbody>
                        {clientExpenseData.expenses && clientExpenseData.expenses.length > 0 ? (
                          clientExpenseData.expenses.map(exp => (
                            <tr key={exp._id} className="hover:bg-slate-50/30 border-b border-slate-100 last:border-0">
                              <td className="px-4 py-3.5 text-xs text-slate-600 font-semibold whitespace-nowrap">
                                {formatDate(exp.date)}
                              </td>
                              <td className="px-4 py-3.5 text-sm text-slate-800 font-bold">
                                {exp.name}
                                {exp.description && <span className="block text-xs font-normal text-slate-500">{exp.description}</span>}
                              </td>
                              <td className="px-4 py-3.5">
                              ...
                              <td className="px-4 py-3.5">
                                {exp.receipt ? (
                                  <button
                                    onClick={() => setReceiptModalUrl(exp.receipt)}
                          ...
                          <tr>
                            <td colSpan="5">
                              <div className="text-center py-10 text-slate-400 text-xs font-semibold">
                                No expenses logged against this client yet.
                              </div>
                            </td>
                          </tr>
                        )}
            ...
            <div className="bg-white rounded-2xl border border-slate-200/80 p-16 text-center text-slate-400">
              ...
              <p className="text-sm font-bold text-slate-600">Select a client from the dropdown above</p>
              <p className="text-xs text-slate-400 mt-1">You will see complete analytics and expense breakdown for that client</p>
            </div>
          )}
        </div>
      )}
